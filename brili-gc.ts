...
export class Heap<X> {
    ...
    private readonly ref_map: Map<number, Array<number>>
    ...
    modify_ref_count(base:number, amt:number) {
      if (this.ref_count.has(base)) {
        let val = this.ref_count.get(base);
        this.ref_count.set(base, val + amt);
      } else {
        this.ref_count.set(base, amt);
      }
    }

    forget_ref_count(base:number) {
      this.ref_count.set(base, 0);
    }

    unit_ref_count(base:number) {
      this.ref_count.set(base, 1);
    }

    read_ref_count() {
      return new Map(this.ref_count);
    }

    collect(base:number) {
      if (this.ref_count.get(base) === 0) {
        this.free(new Key(base, 0));
        this.ref_count.delete(base);
        if (this.ref_map.has(base)) {
          for (let succ of this.ref_map.get(base)) {
            this.modify_ref_count(succ, -1);
            this.collect(succ);
          }
        }
      }
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  if (!(instr.op === "store") && !(instr.op === "load") && !(instr.op === "ptradd") && !(instr.op === "print") && !(instr.op === "jmp") && !(instr.op === "br") && !(instr.op === "ret") && !(instr.op === "nop") && !(instr.op === "id")) {
    if (state.env.has(instr.dest)) {
      if (state.env.get(instr.dest).hasOwnProperty("loc")) {
        //console.error(`Overwrite - Freeing`);
        state.heap.modify_ref_count(state.env.get(instr.dest).loc.base, -1);
        state.heap.collect(state.env.get(instr.dest).loc.base);
      }
    }
  }
  ...
  case "id": {
    ...
    if (val.hasOwnProperty("loc")) {
      //console.error(`ID - incrementing ${val.loc.base} - dest is ${instr.dest}`);
      state.heap.modify_ref_count(val.loc.base, 1);
    }
    if (state.env.has(instr.dest)) {
      if (state.env.get(instr.dest).hasOwnProperty("loc")) {
        //console.error(`Overwrite - Freeing`);
  ...
  case "alloc": {
    ...
    state.heap.modify_ref_count(ptr.loc.base, 1);

    //let heap_after_alloc = state.heap.read_storage().keys();
    //for (let val3 of heap_after_alloc) {
      //console.error(`After alloc - base is ${val3}`);
    //}
    return NEXT;
  }
...
function evalFunc(func: bril.Function, state: State): Value | null {
  let heap_before_func = state.heap.read_storage();
  let before = heap_before_func.keys();
  //for (let val of before) {
  //  console.error(`Before has key ${val}`);
  //}
  //for (let val of before) {
  //  console.error(`Before has key ${val}`);
  //}
  for (let i = 0; i < func.instrs.length; ++i) {
    //for (let x of before) {
    //  console.error(`Instruction - before element is ${x}`);
    //}
    let line = func.instrs[i];
    if ('op' in line) {
      // Run an instruction.
      let action = evalInstr(line, state);
      let heap_after_func2 = state.heap.read_storage().keys();
      //let xyz = [...heap_after_instr]
      //for (let val4 of heap_after_instr) {
      //  console.error(`After instr ${i} - base is ${val4}`);
      //}
      ...
      case 'end': {
        ...
        let list_before = [...before];
        //console.error(`Return - list_before is ${list_before}`);
        if (action.ret === null) {
          //console.error(`Null - No loc!`);
          for (let val of heap_after_func2) {
            //console.error(`Null Returning no loc - Checking base ${val}`);
            if (!list_before.includes(val)) {
              //console.error(`Null Returning no loc - Gotcha! removing base ${val}`);
              state.heap.forget_ref_count(val);
              state.heap.collect(val);
            }
          }
        }
        else if (!action.ret.hasOwnProperty("loc")) {
          //console.error(`No loc!`);
          for (let val of heap_after_func2) {
            //console.error(`Returning no loc - Checking base ${val}`);
            if (!list_before.includes(val)) {
              //console.error(`Returning no loc - Gotcha! removing base ${val}`);
              state.heap.forget_ref_count(val);
              state.heap.collect(val);
            }
          }
        } else {
          //console.error(`Has loc!`);
          for (let val of heap_after_func2) {
            //console.error(`Returning with loc - Checking base ${val}`);
            //console.error(`${list_before.includes(val)}`)
            //console.error(`${val === action.ret.loc.base}`)
            if (!(list_before.includes(val)) && !(val === action.ret.loc.base)) {
              //console.error(`Returning with loc - Gotcha! removing base ${val}`);
              state.heap.forget_ref_count(val);
              state.heap.collect(val);
            } else if (val === action.ret.loc.base) {
              state.heap.unit_ref_count(val);
            }
          }
        }
  ...
  let heap_after_func = state.heap.read_storage().keys();

  let list_before = [...before];
  //console.error(`After starting - list_before is ${list_before}`);
  //console.error(`After starting - heap_after_func is ${heap_after_func}`);
  for (let val of heap_after_func) {
    //console.error(`Checking base ${val}`);
    if (!list_before.includes(val)) {
      //console.error(`Gotcha! removing base ${val}`);
      state.heap.forget_ref_count(val);
      state.heap.collect(val);
    }
  }
...
function evalProg(prog: bril.Program) {
  ...
  if (profiling) {
    let remaining_heap = [...heap.read_storage().keys()];
    console.error(`remaining_heap: ${remaining_heap.length}`);
  }

  for (let val of heap.read_storage().keys()) {
    console.error(`heap has key ${val}`)
  }
  if (!heap.isEmpty()) {
